...
const hyrule = process.argv.includes("--hyrule"), oot = process.argv.includes("--oot");
const local = hyrule || oot, kind = oot ? "oot" : "hyrule";
...
const provider = local ? new AtlasProvider(`.local/${kind}`) : new MapProvider({ ...defaultConfig, format: "raster", tileURL:"https://tile.openstreetmap.de/{z}/{x}/{y}.png", cache: live ? ".local/cache.sqlite" : ":memory:" }, live ? fetch : (async url => {
...
const service = local ? new MapService({ ...defaultConfig, format: "raster", tileURL:"https://tile.openstreetmap.de/{z}/{x}/{y}.png", cache: ":memory:", atlas: ".local/hyrule", atlases: { hyrule: ".local/hyrule", oot: ".local/oot" }, kind }, async () => new Response(fixture.toBuffer("image/png"))) : undefined;
...
async function shot(name: string) { await Bun.write(`dist/qa/${local ? `${kind}-` : live ? "live-" : "replay-"}${name}.png`, encodePNG(wasm.render().slice(), 400, 480)); }
...
if (local) {
  ...
  check(s.planar() && s.info()?.local, "Local atlas opens with its own home and zoom bounds");
  ...
  check(!s.planar() && s.front()!.tiles.every(t => t.input.source === s.info()!.source), "Local atlas switches to synthetic OSM without restarting the guest or daemon");
  await tap(180, 18); await tap(130, 53 + s.maps().findIndex(m => m.kind === kind) * 30); await frames(100);
  check(s.planar() && s.camera.view().x === resume.x && s.camera.view().zoom === resume.zoom, "Switching back restores local atlas coordinates and zoom");
  s.setMode("layers"); await frames(2); await shot("layers"); s.choose(oot ? 1 : 2); await frames(100);
  if (!oot) check(s.annotations.layer() === "collectibles" && s.annotations.rows().every(m => m[2] !== "seed" && m[2] !== "treasure"), "Koroks and treasures remain hidden below their useful zoom range");
...
const receipt = { mode: local ? `complete local ${kind} atlas, compiled guest + Wasm` : live ? "live OSM DE / Photon, compiled guest + Wasm" : "deterministic synthetic provider, compiled guest + Wasm", frames: tick, checks, maxPending, maxResident, maxStaging, ...(service ? service.diagnostics() : provider.diagnostics()),
  hardwareAcceptance: "pending", performance: "Replay validates behavior and budgets, not device frame time" };
await Bun.write(`dist/qa/${local ? kind : live ? "live" : "replay"}.json`, JSON.stringify(receipt, null, 2)); console.log(receipt); service?.close(); provider.close();
